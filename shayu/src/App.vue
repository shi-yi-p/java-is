...
async function submit() {
  ...
  if (!acc) return toast('请输入账户')
  const accOk = /^1[3-9]\d{9}$/.test(acc)
    || /^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(acc)
    || /^[A-Za-z0-9_]{3,20}$/.test(acc)
  if (!accOk) return toast('账户需为手机号 / 邮箱，或 3-20 位用户名')
  ...
  try {
    ...
    if (res.ok) {
      toast(data.message || '注册成功！欢迎加入鲨鱼音乐 🦈🎵')
      setTimeout(() => {
        account.value = ''
        password.value = ''
      ...
      toast(data.message || '该账户已被注册 🐟')
    } else {
      toast(data.message || '注册失败，请稍后再试')
    }
  } catch (e) {
    toast('无法连接服务器，请确认后端已启动 (8081) 🔌')
  }
}
...
async function login() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      try { localStorage.setItem('shark_token', token) } catch (e) {}
      toast('登录成功 🎶（演示 token: ' + token.slice(0, 12) + '…）')
      password.value = ''
    } else if (res.status === 401) {
      toast(data.message || '账户或密码错误')
    } else {
      toast(data.message || '登录失败，请稍后再试')
    }
  } catch (e) {
    toast('无法连接服务器，请确认后端已启动 (8081) 🔌')
  }
...
<template>
  ...
  <main class="shell">
    ...
    <!-- ================= 右：注册表单 ================= -->
    <section class="panel">
      <h1>{{ mode === 'register' ? '创建' : '登录' }}<span>{{ mode === 'register' ? '新账号' : '你的账号' }}</span></h1>
      <p class="sub">{{ mode === 'register' ? '加入鲨鱼音乐，开启你的音乐之旅 🎶' : '欢迎回到鲨鱼音乐 🎧' }}</p>
      ...
      <form @submit.prevent="submit">
        ...
        <div class="field">
          ...
          <div class="control">
            ...
            <input id="password" :type="showPassword ? 'text' : 'password'" v-model="password" :placeholder="mode === 'register' ? '请输入密码（至少 8 位）' : '请输入密码'" :autocomplete="mode === 'register' ? 'new-password' : 'current-password'" />
        ...
        <p class="foot" v-if="mode === 'register'">已经有账号了？<a href="#" @click.prevent="goLogin">立即登录</a></p>
        <p class="foot" v-else>还没有账号？<a href="#" @click.prevent="goRegister">立即注册</a></p>
      </form>
    </section>
  </main>
